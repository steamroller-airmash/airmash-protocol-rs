...
impl ScalarSpec {
	...
	fn de(&self, de: &mut AirmashDeserializerV5) -> SResult<f32> {
		Ok((((de.deserialize_u16()? as i32) - self.shift) as f32) * (1.0 / self.mult))
	}
	fn ser(&self, ser: &mut AirmashSerializerV5, value: f32) -> SResult {
		ser.serialize_u16(
			((value * self.mult) as i32 + self.shift).clamp(0, u16::MAX as i32) as u16,
		)
	}

	fn de_u8(&self, de: &mut AirmashDeserializerV5) -> SResult<f32> {
		Ok((((de.deserialize_u8()? as i32) - self.shift) as f32) * (1.0 / self.mult))
	}
	fn ser_u8(&self, ser: &mut AirmashSerializerV5, value: f32) -> SResult {
...
impl<'ser> AirmashSerializerV5<'ser> {
	...
	pub fn serialize<T: SerializeV5>(&mut self, value: &T) -> SResult {
		value.serialize(self)
	}

	pub fn serialize_bytes(&mut self, bytes: &[u8]) -> SResult {
		self.data.extend_from_slice(bytes);
		Ok(())
	}

	pub fn serialize_u8(&mut self, value: u8) -> SResult {
		self.serialize_bytes(&[value])
	}
	pub fn serialize_u16(&mut self, value: u16) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}
	pub fn serialize_u32(&mut self, value: u32) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}
	pub fn serialize_u64(&mut self, value: u64) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}
	pub fn serialize_u128(&mut self, value: u128) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}

	pub fn serialize_i8(&mut self, value: i8) -> SResult {
		self.serialize_u8(value as u8)
	}
	pub fn serialize_i16(&mut self, value: i16) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}
	pub fn serialize_i32(&mut self, value: i32) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}
	pub fn serialize_i64(&mut self, value: i64) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}
	pub fn serialize_i128(&mut self, value: i128) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}

	pub fn serialize_f32(&mut self, value: f32) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}
	pub fn serialize_f64(&mut self, value: f64) -> SResult {
		self.serialize_bytes(&value.to_le_bytes())
	}

	pub fn serialize_u24(&mut self, value: u32) -> SResult {
		self.serialize_bytes(&value.to_le_bytes()[..3])
	}

	pub fn serialize_array_small<T>(&mut self, data: &[T]) -> SResult
	where
		T: SerializeV5,
	{
	...
	pub fn serialize_array_large<T>(&mut self, data: &[T]) -> SResult
	where
		T: SerializeV5,
	{
	...
	pub fn serialize_text_small<T>(&mut self, data: &BStr) -> SResult {
		if data.len() > u8::MAX as usize {
			return Err(Error::new(ErrorKind::ArraySizeTooLarge));
		}
	...
	pub fn serialize_text_large<T>(&mut self, data: &BStr) -> SResult {
		if data.len() > u16::MAX as usize {
			return Err(Error::new(ErrorKind::ArraySizeTooLarge));
		}
	...
	pub fn serialize_accel(&mut self, v: Vector2<f32>) -> SResult {
		ACCEL_SPEC.ser(self, v[0])?;
		ACCEL_SPEC.ser(self, v[1])
	}
	pub fn serialize_low_res_pos(&mut self, pos: Option<Vector2<f32>>) -> SResult {
		let x = pos
	...
	pub fn serialize_pos_f32(&mut self, pos: Vector2<f32>) -> SResult {
		self.serialize_f32(pos[0])?;
		self.serialize_f32(pos[1])
	}
	pub fn serialize_pos(&mut self, pos: Vector2<f32>) -> SResult {
		self.serialize_coordx(pos[0])?;
		self.serialize_coordy(pos[1])
	}
	pub fn serialize_pos24(&mut self, pos: Vector2<f32>) -> SResult {
		self.serialize_coord24(pos[0])?;
		self.serialize_coord24(pos[1])
	}
	pub fn serialize_vel(&mut self, pos: Vector2<f32>) -> SResult {
		self.serialize_speed(pos[0])?;
		self.serialize_speed(pos[1])
	}

	pub fn serialize_coord24(&mut self, v: f32) -> SResult {
		let spec = COORD24_SPEC;
		self.serialize_u24(
			((v * spec.mult) as i32 + spec.shift).clamp(0, (u32::MAX >> 8) as i32) as u32,
		)
	}
	pub fn serialize_coordx(&mut self, v: f32) -> SResult {
		COORDX_SPEC.ser(self, v)
	}
	pub fn serialize_coordy(&mut self, v: f32) -> SResult {
		COORDY_SPEC.ser(self, v)
	}
	pub fn serialize_energy(&mut self, v: f32) -> SResult {
		ENERGY_SPEC.ser_u8(self, v)
	}
	pub fn serialize_regen(&mut self, v: f32) -> SResult {
		REGEN_SPEC.ser(self, v)
	}
	pub fn serialize_rotation(&mut self, v: f32) -> SResult {
		ROTATION_SPEC.ser(self, v)
	}
	pub fn serialize_speed(&mut self, v: f32) -> SResult {
...
impl<'de> AirmashDeserializerV5<'de> {
	...
	pub fn deserialize<T: DeserializeV5<'de>>(&mut self) -> SResult<T> {
		T::deserialize(self)
	}

	pub fn deserialize_fixed<const N: usize>(&mut self) -> SResult<[u8; N]> {
	...
	pub fn deserialize_bytes(&mut self, len: usize) -> SResult<&'de [u8]> {
	...
	pub fn deserialize_u8(&mut self) -> SResult<u8> {
	...
	pub fn deserialize_u16(&mut self) -> SResult<u16> {
		self.deserialize_fixed().map(u16::from_le_bytes)
	}
	pub fn deserialize_u32(&mut self) -> SResult<u32> {
		self.deserialize_fixed().map(u32::from_le_bytes)
	}
	pub fn deserialize_u64(&mut self) -> SResult<u64> {
		self.deserialize_fixed().map(u64::from_le_bytes)
	}
	pub fn deserialize_u128(&mut self) -> SResult<u128> {
		self.deserialize_fixed().map(u128::from_le_bytes)
	}

	pub fn deserialize_i8(&mut self) -> SResult<i8> {
		self.deserialize_u8().map(|x| x as i8)
	}
	pub fn deserialize_i16(&mut self) -> SResult<i16> {
		self.deserialize_fixed().map(i16::from_le_bytes)
	}
	pub fn deserialize_i32(&mut self) -> SResult<i32> {
		self.deserialize_fixed().map(i32::from_le_bytes)
	}
	pub fn deserialize_i64(&mut self) -> SResult<i64> {
		self.deserialize_fixed().map(i64::from_le_bytes)
	}
	pub fn deserialize_i128(&mut self) -> SResult<i128> {
		self.deserialize_fixed().map(i128::from_le_bytes)
	}

	pub fn deserialize_f32(&mut self) -> SResult<f32> {
		self.deserialize_fixed().map(f32::from_le_bytes)
	}
	pub fn deserialize_f64(&mut self) -> SResult<f64> {
		self.deserialize_fixed().map(f64::from_be_bytes)
	}

	pub fn deserialize_u24(&mut self) -> SResult<u32> {
		let [a, b, c] = self.deserialize_fixed()?;
		Ok(u32::from_le_bytes([a, b, c, 0]))
	}

	pub fn deserialize_array_small<T>(&mut self) -> SResult<Vec<T>>
	where
		T: DeserializeV5<'de>,
	{
	...
	pub fn deserialize_array_large<T>(&mut self) -> SResult<Vec<T>>
	where
		T: DeserializeV5<'de>,
	{
	...
	pub fn deserialize_text_small<T>(&mut self) -> SResult<&'de BStr>
	where
		T: DeserializeV5<'de>,
	{
		let len = self.deserialize_u8()? as usize;
	...
	pub fn deserialize_text_large<T>(&mut self) -> SResult<&'de BStr>
	where
		T: DeserializeV5<'de>,
	{
		let len = self.deserialize_u16()? as usize;
	...
	pub fn deserialize_accel(&mut self) -> SResult<Vector2<f32>> {
		Ok(Vector2::new(ACCEL_SPEC.de(self)?, ACCEL_SPEC.de(self)?))
	}
	pub fn deserialize_low_res_pos(&mut self) -> SResult<Option<Vector2<f32>>> {
	...
	pub fn deserialize_pos_f32(&mut self) -> SResult<Vector2<f32>> {
		Ok(Vector2::new(
			self.deserialize_f32()?,
			self.deserialize_f32()?,
		))
	}
	pub fn deserialize_pos(&mut self) -> SResult<Vector2<f32>> {
		Ok(Vector2::new(
			self.deserialize_coordx()?,
			self.deserialize_coordy()?,
		))
	}
	pub fn deserialize_pos24(&mut self) -> SResult<Vector2<f32>> {
		Ok(Vector2::new(
			self.deserialize_coord24()?,
			self.deserialize_coord24()?,
		))
	}
	pub fn deserialize_vel(&mut self) -> SResult<Vector2<f32>> {
		Ok(Vector2::new(
	...
	pub fn deserialize_coord24(&mut self) -> SResult<f32> {
		let spec = COORD24_SPEC;
		Ok((((self.deserialize_u24()? as i32) - spec.shift) as f32) * (1.0 / spec.mult))
	}
	pub fn deserialize_coordx(&mut self) -> SResult<f32> {
		COORDX_SPEC.de(self)
	}
	pub fn deserialize_coordy(&mut self) -> SResult<f32> {
		COORDY_SPEC.de(self)
	}
	pub fn deserialize_energy(&mut self) -> SResult<f32> {
		ENERGY_SPEC.de_u8(self)
	}
	pub fn deserialize_regen(&mut self) -> SResult<f32> {
		REGEN_SPEC.de(self)
	}
	pub fn deserialize_rotation(&mut self) -> SResult<f32> {
		ROTATION_SPEC.de(self)
	}
	pub fn deserialize_speed(&mut self) -> SResult<f32> {
		SPEED_SPEC.de(self)
	}
}

pub trait SerializeV5 {
	fn serialize<'ser>(&self, ser: &mut AirmashSerializerV5<'ser>) -> SResult;
}

pub trait DeserializeV5<'de>: Sized {
	fn deserialize(de: &mut AirmashDeserializerV5<'de>) -> SResult<Self>;
}
